export const cardsData = [
  {
    id: 1,
    images: [
      {
        id: 1,
        image:
          "https://a0.muscache.com/im/pictures/miso/Hosting-45465864/original/3d966c94-4c87-479b-8eeb-4889e9fb6ac9.jpeg?im_w=720",
      },
      {
        id: 2,
        image:
          "https://a0.muscache.com/im/pictures/miso/Hosting-45465864/original/eeffdea4-bbcf-4a05-bcb9-579a03bf41ab.jpeg?im_w=720",
      },
      {
        id: 3,
        image:
          "https://a0.muscache.com/im/pictures/miso/Hosting-45465864/original/9d4c4d58-b9e6-4a2d-9883-eb2df68c0ba0.jpeg?im_w=720",
      },
      {
        id: 4,
        image:
          "https://a0.muscache.com/im/pictures/miso/Hosting-45465864/original/3c44f9fd-0a88-41dd-acb5-ebf58bde739f.jpeg?im_w=720",
      },
      {
        id: 5,
        image:
          "https://a0.muscache.com/im/pictures/miso/Hosting-45465864/original/39cdf5aa-a5d6-4f55-893c-73bf310dd598.jpeg?im_w=720",
      },
      {
        id: 6,
        image:
          "https://a0.muscache.com/im/pictures/miso/Hosting-45465864/original/9f680175-524d-47f7-9814-a88b9bc42096.jpeg?im_w=720",
      },
      {
        id: 7,
        image:
          "https://a0.muscache.com/im/pictures/miso/Hosting-45465864/original/4e9d046b-0499-4d12-ada5-401fa81102da.jpeg?im_w=720",
      },
    ],
    carousel: [
      {
        id: 1,
        image:
          "https://a0.muscache.com/im/pictures/miso/Hosting-45465864/original/3d966c94-4c87-479b-8eeb-4889e9fb6ac9.jpeg?im_w=720",
      },
      {
        id: 2,
        image:
          "https://a0.muscache.com/im/pictures/miso/Hosting-45465864/original/eeffdea4-bbcf-4a05-bcb9-579a03bf41ab.jpeg?im_w=720",
      },
      {
        id: 3,
        image:
          "https://a0.muscache.com/im/pictures/miso/Hosting-45465864/original/9d4c4d58-b9e6-4a2d-9883-eb2df68c0ba0.jpeg?im_w=720",
      },
      {
        id: 4,
        image:
          "https://a0.muscache.com/im/pictures/miso/Hosting-45465864/original/3c44f9fd-0a88-41dd-acb5-ebf58bde739f.jpeg?im_w=720",
      },
      {
        id: 5,
        image:
          "https://a0.muscache.com/im/pictures/miso/Hosting-45465864/original/39cdf5aa-a5d6-4f55-893c-73bf310dd598.jpeg?im_w=720",
      },
      {
        id: 6,
        image:
          "https://a0.muscache.com/im/pictures/miso/Hosting-45465864/original/39cdf5aa-a5d6-4f55-893c-73bf310dd598.jpeg?im_w=720",
      },
    ],
    title: "Container Suite: suspended between land and sea",
    hostedbytitle: "Earthen home hosted by Vittorio Emanuele",
    titleIcon: require("../../../assets/icons/titleicon.png"),
    rating: 4.91,
    reviews: 23,
    city: "Terrasini, Sicilia, Italy",
    hostedby: `https://a0.muscache.com/im/pictures/user/82b4c3a0-d592-4cc8-b12b-06969fbee837.jpg?im_w=240`,
    description: `Featured in Conde Nast Traveler`,
    description1Icon: require("../../../assets/icons/featuredin.png"),
    description1Title: `Featured in`,
    description1: `Featured in Conde Nast Traveler,June 2021`,
    description2Title: `Superhosts are experienced, highly rated hosts who are committed to providing great stays for guests.`,
    description2Icon: require("../../../assets/icons/superhost.png"),
    description2: `Vittorio Emanuele is a Superhost`,
    description3Icon: require("../../../assets/icons/keyicon.png"),
    description3: `Great check-in experience`,
    description3Title: `94% of recent guests gave the check-in process a 5-star rating.`,
    aircoverlogo: `https://a0.muscache.com/im/pictures/54e427bb-9cb7-4a81-94cf-78f19156faad.jpg`,
    aircoverdescription: `Every booking includes free protection from Host cancellations, listing inaccuracies, and other issues like trouble checking in.`,
    placeOffers: [
      {
        id: 1,
        title: `Wifi`,
        icon: require("../../../assets/icons/wifi.png"),
      },
      {
        id: 2,
        title: `Free parking on premises`,
        icon: require("../../../assets/icons/parking.png"),
      },
      {
        id: 3,
        title: `Private bot tub`,
        icon: require("../../../assets/icons/privatebathtub.png"),
      },
      {
        id: 4,
        title: `Air conditioning`,
        icon: require("../../../assets/icons/aircondition.png"),
      },
      {
        id: 5,
        title: `Bathtub`,
        icon: require("../../../assets/icons/bathtub.png"),
      },
      {
        id: 6,
        title: `Private patio or balcony`,
        icon: require("../../../assets/icons/balcony.png"),
      },
      {
        id: 7,
        title: `Private backyard – Fully fenced`,
        icon: require("../../../assets/icons/backyard.png"),
      },
      {
        id: 8,
        title: `Security cameras on property`,
        icon: require("../../../assets/icons/securitycamera.png"),
      },
      {
        id: 9,
        title: `Carbon monoxide alarm`,
        icon: require("../../../assets/icons/nocoalarm.png"),
      },
      {
        id: 10,
        title: `Smoke alarm`,
        icon: require("../../../assets/icons/nosmokealarm.png"),
      },
    ],
    date: "Jun 7-13",
    price: "$367",
    numOfGuests: 2,
    bedrooms: 1,
    beds: 1,
    baths: 1,
  },
  {
    id: 2,
    images: [
      {
        id: 1,
        image:
          "https://a0.muscache.com/im/pictures/d89a974e-73db-4ba6-b2f4-8e1876534731.jpg?im_w=720",
      },
      {
        id: 2,
        image:
          "https://a0.muscache.com/im/pictures/4dd0aa84-b8e0-49cf-bbde-ee08df7ae452.jpg?im_w=720",
      },
      {
        id: 3,
        image:
          "https://a0.muscache.com/im/pictures/ad429202-910a-400b-9538-4bf3af96378c.jpg?im_w=720",
      },
      {
        id: 4,
        image:
          "https://a0.muscache.com/im/pictures/d953d715-89ca-4c10-93c5-1f8e2625a5cb.jpg?im_w=720",
      },
      {
        id: 5,
        image:
          "https://a0.muscache.com/im/pictures/0a3c1bc6-b7bb-4b97-b44e-b8532f5afcec.jpg?im_w=720",
      },
    ],
    carousel: [
      {
        id: 1,
        image:
          "https://a0.muscache.com/im/pictures/d89a974e-73db-4ba6-b2f4-8e1876534731.jpg?im_w=720",
      },
      {
        id: 2,
        image:
          "https://a0.muscache.com/im/pictures/4dd0aa84-b8e0-49cf-bbde-ee08df7ae452.jpg?im_w=720",
      },
      {
        id: 3,
        image:
          "https://a0.muscache.com/im/pictures/ad429202-910a-400b-9538-4bf3af96378c.jpg?im_w=720",
      },
      {
        id: 4,
        image:
          "https://a0.muscache.com/im/pictures/d953d715-89ca-4c10-93c5-1f8e2625a5cb.jpg?im_w=720",
      },
      {
        id: 5,
        image:
          "https://a0.muscache.com/im/pictures/0a3c1bc6-b7bb-4b97-b44e-b8532f5afcec.jpg?im_w=720",
      },
    ],
    title: "Swim Under a Private Waterfall at a Tranquil Forest Haven",
    hostedbytitle: "Entire home hosted by Marko",
    titleIcon: require("../../../assets/icons/titleicon.png"),
    rating: 4.9,
    reviews: 220,
    city: "Paraty, Rio de Janeiro, Brazil",
    hostedby: `https://a0.muscache.com/im/pictures/user/7efcfb1f-beeb-45c7-8194-99bf0b249c69.jpg?im_w=240`,
    description: `Designed by Atelier Marko Brajovic`,
    description1Icon: require("../../../assets/icons/designedin.png"),
    description1Title: `Designed by`,
    description1: `Atelier Marko Brajovic`,
    description2Icon: require("../../../assets/icons/featuredin.png"),
    description2Title: `Featured in`,
    description2: `ArchDaily, January 2022
    Dwell, March 2021`,
    description3Icon: require("../../../assets/icons/freecancel.png"),
    description3Title: `Free cancellation before Jul 2.`,
    description3: ``,
    // aircoverlogo: `https://a0.muscache.com/im/pictures/54e427bb-9cb7-4a81-94cf-78f19156faad.jpg`,
    // aircoverdescription: `Every booking includes free protection from Host cancellations, listing inaccuracies, and other issues like trouble checking in.`,
    placeOffers: [
      {
        id: 1,
        title: `Wifi`,
        icon: require("../../../assets/icons/wifi.png"),
      },
      {
        id: 2,
        title: `Free parking on premises`,
        icon: require("../../../assets/icons/parking.png"),
      },
      {
        id: 3,
        title: `Private bot tub`,
        icon: require("../../../assets/icons/privatebathtub.png"),
      },
      {
        id: 4,
        title: `Air conditioning`,
        icon: require("../../../assets/icons/aircondition.png"),
      },
      {
        id: 5,
        title: `Bathtub`,
        icon: require("../../../assets/icons/bathtub.png"),
      },
      {
        id: 6,
        title: `Private patio or balcony`,
        icon: require("../../../assets/icons/balcony.png"),
      },
      {
        id: 7,
        title: `Private backyard – Fully fenced`,
        icon: require("../../../assets/icons/backyard.png"),
      },
      {
        id: 8,
        title: `Security cameras on property`,
        icon: require("../../../assets/icons/securitycamera.png"),
      },
      {
        id: 9,
        title: `Carbon monoxide alarm`,
        icon: require("../../../assets/icons/nocoalarm.png"),
      },
      {
        id: 10,
        title: `Smoke alarm`,
        icon: require("../../../assets/icons/nosmokealarm.png"),
      },
    ],
    date: "Jun 7-13",
    price: "$367",
    numOfGuests: 6,
    bedrooms: 2,
    beds: 2,
    baths: 1,
  },
  // {
  //   id: 3,
  //   images: [
  //     {
  //       id: 1,
  //       image:
  //         "https://a0.muscache.com/im/pictures/monet/Select-34444025/original/944d56fa-e9a6-48fb-a9c5-e4e3778042d7?im_w=720",
  //     },
  //     {
  //       id: 2,
  //       image:
  //         "https://a0.muscache.com/im/pictures/monet/Select-34444025/original/622a7f24-1a55-4468-82b5-fb3abc83020f?im_w=720",
  //     },
  //     {
  //       id: 3,
  //       image:
  //         "https://a0.muscache.com/im/pictures/monet/Select-34444025/original/68e860ea-a095-4651-b911-0394d9844316?im_w=720",
  //     },
  //     {
  //       id: 4,
  //       image:
  //         "https://a0.muscache.com/im/pictures/monet/Select-34444025/original/5b038cc8-222e-43ba-8aef-187efe91e787?im_w=720",
  //     },
  //     {
  //       id: 5,
  //       image:
  //         "https://a0.muscache.com/im/pictures/monet/Select-34444025/original/9e8d63dc-a594-4364-9be4-daf83610fd8c?im_w=720",
  //     },
  //   ],
  //   carousel: [
  //     {
  //       id: 1,
  //       image:
  //         "https://a0.muscache.com/im/pictures/monet/Select-34444025/original/944d56fa-e9a6-48fb-a9c5-e4e3778042d7?im_w=720",
  //     },
  //     {
  //       id: 2,
  //       image:
  //         "https://a0.muscache.com/im/pictures/monet/Select-34444025/original/622a7f24-1a55-4468-82b5-fb3abc83020f?im_w=720",
  //     },
  //     {
  //       id: 3,
  //       image:
  //         "https://a0.muscache.com/im/pictures/monet/Select-34444025/original/68e860ea-a095-4651-b911-0394d9844316?im_w=720",
  //     },
  //     {
  //       id: 4,
  //       image:
  //         "https://a0.muscache.com/im/pictures/monet/Select-34444025/original/5b038cc8-222e-43ba-8aef-187efe91e787?im_w=720",
  //     },
  //     {
  //       id: 5,
  //       image:
  //         "https://a0.muscache.com/im/pictures/monet/Select-34444025/original/9e8d63dc-a594-4364-9be4-daf83610fd8c?im_w=720",
  //     },
  //   ],
  //   title: "The WonderINN Mirrored Glass Cabin",
  // hostedbytitle: "Tiny home hosted by Jeremy & Erle",
  // titleIcon: require('../../../assets/icons/titleicon.png'),
  //   rating: 4.95,
  //   reviews: 199,
  //   city: "Rælingen, Akershus, Norway",
  //   date: "Jun 9-15",
  //   price: "$431",
  //   hostedby: `https://a0.muscache.com/im/pictures/user/e016c6a2-bf27-4315-91d0-903eb093620d.jpg?im_w=240`,
  //   description: `Featured in Venuereport`,
  //   description1Icon: require("../../../assets/icons/featuredin.png"),
  //   description1Title: `Featured in`,
  //   description1: `Venuereport, May 2021
  //   Bonytt, May 2020`,
  //   description2Title: `Designed by`,
  //   description2Icon: require("../../../assets/icons/designedin.png"),
  //   description2: `Jeremy & Erle`,
  //   description3Icon: require("../../../assets/icons/freecancel.png"),
  //   description3: ``,
  //   description3Title: `Free cancellation for 48 hours.`,
  //   aircoverlogo: `https://a0.muscache.com/im/pictures/54e427bb-9cb7-4a81-94cf-78f19156faad.jpg`,
  //   aircoverdescription: `Every booking includes free protection from Host cancellations, listing inaccuracies, and other issues like trouble checking in.`,
  //   placeOffers: [
  //     {
  //       id: 1,
  //       title: `Wifi`,
  //       icon: require("../../../assets/icons/wifi.png"),
  //     },
  //     {
  //       id: 2,
  //       title: `Free parking on premises`,
  //       icon: require("../../../assets/icons/parking.png"),
  //     },
  //     {
  //       id: 3,
  //       title: `Private bot tub`,
  //       icon: require("../../../assets/icons/privatebathtub.png"),
  //     },
  //     {
  //       id: 4,
  //       title: `Air conditioning`,
  //       icon: require("../../../assets/icons/aircondition.png"),
  //     },
  //     {
  //       id: 5,
  //       title: `Bathtub`,
  //       icon: require("../../../assets/icons/bathtub.png"),
  //     },
  //     {
  //       id: 6,
  //       title: `Private patio or balcony`,
  //       icon: require("../../../assets/icons/balcony.png"),
  //     },
  //     {
  //       id: 7,
  //       title: `Private backyard – Fully fenced`,
  //       icon: require("../../../assets/icons/backyard.png"),
  //     },
  //     {
  //       id: 8,
  //       title: `Security cameras on property`,
  //       icon: require("../../../assets/icons/securitycamera.png"),
  //     },
  //     {
  //       id: 9,
  //       title: `Carbon monoxide alarm`,
  //       icon: require("../../../assets/icons/nocoalarm.png"),
  //     },
  //     {
  //       id: 10,
  //       title: `Smoke alarm`,
  //       icon: require("../../../assets/icons/nosmokealarm.png"),
  //     },
  //   ],

  //   numOfGuests: 4,
  //   bedrooms: 2,
  //   beds: 2,
  //   baths: 1,
  // },
  // {
  //   id: 4,
  //   images: [
  //     {
  //       id: 1,
  //       image:
  //         "https://a0.muscache.com/im/pictures/prohost-api/Hosting-577893800532627464/original/c8bf03d8-04d3-4020-9304-5913e93780ee.jpeg?im_w=720",
  //     },
  //     {
  //       id: 2,
  //       image:
  //         "https://a0.muscache.com/im/pictures/prohost-api/Hosting-577893800532627464/original/81bba94f-0869-4fe5-afa0-73311052553d.jpeg?im_w=720",
  //     },
  //     {
  //       id: 3,
  //       image:
  //         "https://a0.muscache.com/im/pictures/prohost-api/Hosting-577893800532627464/original/5072d1cb-dbdf-4c42-9336-c16f807bd11f.jpeg?im_w=720",
  //     },
  //     {
  //       id: 4,
  //       image:
  //         "https://a0.muscache.com/im/pictures/prohost-api/Hosting-577893800532627464/original/c19d60ca-ffea-44f3-b3ee-386c2eb8ef41.jpeg?im_w=720",
  //     },
  //     {
  //       id: 5,
  //       image:
  //         "https://a0.muscache.com/im/pictures/prohost-api/Hosting-577893800532627464/original/6ba49dcf-51c8-4368-a6c8-bfaa52c02b3d.jpeg?im_w=720",
  //     },
  //     {
  //       id: 6,
  //       image:
  //         "https://a0.muscache.com/im/pictures/prohost-api/Hosting-577893800532627464/original/e5917c73-7255-477f-b8bc-f8433e07e892.jpeg?im_w=720",
  //     },
  //     {
  //       id: 7,
  //       image:
  //         "https://a0.muscache.com/im/pictures/prohost-api/Hosting-577893800532627464/original/0cc7ff14-682c-4b3f-b12e-edf4562a5a7e.jpeg?im_w=720",
  //     },
  //   ],
  //   carousel: [
  //     {
  //       id: 1,
  //       image:
  //         "https://a0.muscache.com/im/pictures/prohost-api/Hosting-577893800532627464/original/c8bf03d8-04d3-4020-9304-5913e93780ee.jpeg?im_w=720",
  //     },
  //     {
  //       id: 2,
  //       image:
  //         "https://a0.muscache.com/im/pictures/prohost-api/Hosting-577893800532627464/original/81bba94f-0869-4fe5-afa0-73311052553d.jpeg?im_w=720",
  //     },
  //     {
  //       id: 3,
  //       image:
  //         "https://a0.muscache.com/im/pictures/prohost-api/Hosting-577893800532627464/original/5072d1cb-dbdf-4c42-9336-c16f807bd11f.jpeg?im_w=720",
  //     },
  //     {
  //       id: 4,
  //       image:
  //         "https://a0.muscache.com/im/pictures/prohost-api/Hosting-577893800532627464/original/c19d60ca-ffea-44f3-b3ee-386c2eb8ef41.jpeg?im_w=720",
  //     },
  //     {
  //       id: 5,
  //       image:
  //         "https://a0.muscache.com/im/pictures/prohost-api/Hosting-577893800532627464/original/6ba49dcf-51c8-4368-a6c8-bfaa52c02b3d.jpeg?im_w=720",
  //     },
  //   ],
  //   title: "Beau Manor",
  // hostedbytitle: "Entire villa hosted by In Residence",
  // titleIcon: require('../../../assets/icons/titleicon.png'),
  //   rating: 4.87,
  //   reviews: 230,
  //   city: "Cape Town, South Africa",
  //   hostedby: `https://a0.muscache.com/im/pictures/user/f53ab502-7f56-4dfc-a589-a7fb16012d1b.jpg?im_w=240`,
  //   description: `8379 kilometeres away`,
  //   date: "Jun 14-19",
  //   price: "$9286",
  //   description1Icon: require("../../../assets/icons/divein.png"),
  //   description1Title: `Dive right in`,
  //   description1: `This is one of the few places in the area with a pool.`,
  //   description2Title: `Experienced host`,
  //   description2Icon: require("../../../assets/icons/superhost.png"),
  //   description2: `In Residence has 30 reviews for other places.`,

  //   aircoverlogo: `https://a0.muscache.com/im/pictures/54e427bb-9cb7-4a81-94cf-78f19156faad.jpg`,
  //   aircoverdescription: `Every booking includes free protection from Host cancellations, listing inaccuracies, and other issues like trouble checking in.`,
  //   placeOffers: [
  //     {
  //       id: 1,
  //       title: `Wifi`,
  //       icon: require("../../../assets/icons/wifi.png"),
  //     },
  //     {
  //       id: 2,
  //       title: `Free parking on premises`,
  //       icon: require("../../../assets/icons/parking.png"),
  //     },
  //     {
  //       id: 3,
  //       title: `Kitchen`,
  //       icon: require("../../../assets/icons/kitchen.png"),
  //     },
  //     {
  //       id: 4,
  //       title: `Private pool`,
  //       icon: require("../../../assets/icons/privatepool.png"),
  //     },
  //     {
  //       id: 5,
  //       title: `TV`,
  //       icon: require("../../../assets/icons/tv.png"),
  //     },
  //     {
  //       id: 6,
  //       title: `Washer`,
  //       icon: require("../../../assets/icons/washer.png"),
  //     },
  //     {
  //       id: 7,
  //       title: `Private backyard – Fully fenced`,
  //       icon: require("../../../assets/icons/backyard.png"),
  //     },
  //     {
  //       id: 8,
  //       title: `Security cameras on property`,
  //       icon: require("../../../assets/icons/securitycamera.png"),
  //     },
  //     {
  //       id: 9,
  //       title: `Carbon monoxide alarm`,
  //       icon: require("../../../assets/icons/nocoalarm.png"),
  //     },
  //     {
  //       id: 10,
  //       title: `Smoke alarm`,
  //       icon: require("../../../assets/icons/nosmokealarm.png"),
  //     },
  //   ],

  //   numOfGuests: 8,
  //   bedrooms: 4,
  //   beds: 4,
  //   baths: 4,
  // },
];
